...
const MainTips: FC = () => {
  ...
  // function that runs when my tips card component mounts, assigning fallback tips until, Ai generated tips are ready
  useEffect(() => {
    if (isLoading) return;
  ...
  return (
    <section className="w-full min:h-[60vh] sm:min-h-[15vh] h-full rounded-sm shadow-md bg-green-600 cursor-arrow hover:-translate-y-1 transition-transform p-4 overflow-y-scroll md:overflow-hidden tips-wrapper flex flex-col gap-y-4">
      ...
      {tips.length !== 0 ? (
        tips.map((tip, index) => (
          <p key={index} className="text-white text-md leading-tight">
            {tip}
          </p>
        ))
      ) : (
